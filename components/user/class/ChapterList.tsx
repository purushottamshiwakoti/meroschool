...
const ChapterList: React.FC<ChapterListProps> = ({
  ...
  const handleSearch = async (e: any) => {
    ...
    try {
      alert(searchValue);
      setLoading(true);
      const response = await prismadb.question.findMany({});
      console.log(response);

      router.push(`${url}/class/${slug}?search=${searchValue}`);
    } catch (e) {
      console.log(e);
    } finally {
      setLoading(false);
      ...
      <div>
        ...
        <div className="flex flex-nowrap space-x-3 lg:space-x-10 w-full overflow-x-scroll mt-[7rem]  md:mt-[9rem]  lg:mt-[9.5rem]">
          {chapters.map((chapter) => (
            <Link href={chapter.name} key={chapter.id}>
      ...
      <div className="">
        ...
        {!loading
          ...
          : Array.from({ length: 3 }, (_, index) => (
              <div
                key={index}
                className="flex items-center bg-white space-x-4 mt-4 p-4 rounded-md shadow-md"
              >
